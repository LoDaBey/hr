...
import { listWorkflowErrors } from '@/lib/repos/workflow-errors';
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const [errors, failed_emails] = await Promise.all([
      listWorkflowErrors({ resolved, limit }),
      listFailedCommunications(limit),
    ]);

    const data: HrErrorsListResult = { errors, failed_emails };
    return jsonOk(data);
  } catch (error) {
    console.error(error);
