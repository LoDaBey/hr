...
export async function POST(
  ...
  try {
    ...
    if (!result.ok) {
      ...
      if (result.reason === 'wrong_stage') {
        return jsonError(
          409,
          'WRONG_STAGE',
          'Application is not ready for a recorded tech test invite',
        );
      }
      return jsonError(404, 'NOT_FOUND', 'No recorded tech test configured for this job');
    }
    ...
    console.error(error);
    return jsonError(500, 'INTERNAL_ERROR', 'Failed to invite candidate to recorded tech test');
  }
}
