  ...
  try {
    ...
    if (!result.ok) {
      if (result.reason === 'not_found') {
        return jsonError(404, 'NOT_FOUND', 'No recorded test sitting found for this candidate');
      }
      if (result.reason === 'not_submitted') {
        return jsonError(409, 'WRONG_STAGE', 'Recorded test is not awaiting grading');
      }
      if (result.reason === 'already_graded') {
        return jsonError(409, 'ALREADY_GRADED', 'Recorded test is already graded');
      }
      return jsonError(409, 'WRONG_STAGE', 'Recorded test is not awaiting grading');
    }
